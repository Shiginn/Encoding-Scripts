...
class Encoder:
    ...
    def clean_up(
        self,
        add_file: PATH | Sequence[PATH] | None = None,
    ...
    def make_comp(self, **comp_args: Any) -> None:
        ...
        lossless = self.file.name_clip_output.append_stem("_lossless")
        filtered = source(f"{lossless}_lossless.mkv", force_lsmas=True) if lossless.exists() else self.clip
        make_comps(
            {
