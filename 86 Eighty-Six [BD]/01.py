...
ed_start = None
ed_offset = -1
title_ranges = [(21985, 22056), (33927, 34045)]
...
class Filtering(EightySixFiltering):
    def scenefilter(self, clip: vs.VideoNode, denoise: vs.VideoNode) -> vs.VideoNode:
        ...
        detail_mask = lvf.mask.detail_mask(denoise, rad=1, brz_a=0.2, brz_b=0.07)
        detail_mask_blur = lvf.mask.detail_mask(denoise, rad=1, brz_a=0.2, brz_b=0.07, sigma=0.8).std.Maximum()

        details = core.std.Expr([detail_mask, detail_mask_blur], expr="x y -").std.Binarize(5<<8).std.Minimum().std.Maximum().std.Maximum().std.Maximum()
        details = lvf.rfs(core.std.BlankClip(details), details, screen_ranges)

        scene_filter = core.std.MaskedMerge(clip, denoise, details)
